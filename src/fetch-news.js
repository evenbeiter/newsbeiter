...
async function biiGetList(siteName,t){
  ...
  if (data.Children){
    for (let c of data.Children){
      ...
      html+=`<p class="title" onclick="getContent('${siteName}',this.id,'${data.Url}')">${data.Title}</p><div id="${data.Url}" class="content fs12" onclick="getContent('${siteName}',this.id,'${data.Url}')">${imgHtml}${shareLink(data.Url)}</div><hr>`;
  }
  }catch{html='<p>No Content.</p>'}
  return html;
}
...
async function ftGetList(siteName,t){
  ...
  items.push([hh[i].querySelector('.o-teaser__heading a').href.replace('https://evenbeiter.github.io',''),hh[i].querySelector('.o-teaser__heading').innerText,tt[i].innerText]);
  }
  
  for (let h of items){
    html+=`<p class="title t-tl" onclick="getContent('${siteName}',this.id,'${h[0]}')">${h[1]}</p><p class="time">${h[2]}</p><div id="${h[0]}" class="content fs12" onclick="getContent('${siteName}',this.id,'${h[0]}')"></div><hr>`
  }
  }catch{html='<p>尚無內容</p>'}
  return html;
}
...
async function ftGetContent(id){
  try{const res = await fetch(preStr+'https://www.ft.com'+id);const str=await res.text();
  const parser = new DOMParser();const doc = parser.parseFromString(str, "text/html");
